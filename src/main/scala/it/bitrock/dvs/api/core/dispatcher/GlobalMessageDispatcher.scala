...
class GlobalMessageDispatcher(val sourceActorRef: ActorRef, kafkaPollerHub: KafkaPollerHub, val webSocketConfig: WebSocketConfig)
  ...
  private def behaviorForFlights(box: CoordinatesBox): Receive = {
    case flights: FlightReceivedList =>
      ...
      forwardMessage(ApiEvent(EventType.from(flights), boxedFlights(flights, box)).toJson.toString)
    case airports: AirportList =>
      logger.debug(s"Got a $airports from Kafka Consumer")
      forwardMessage(ApiEvent(EventType.from(airports), boxedAirports(airports, box)).toJson.toString)
  }
  ...
  private def commands(currentBehaviors: Map[BehaviorType, BehaviorState]): Receive = {
    ...
    case StartTops(updateRate) =>
      ...
      val s = context.system.scheduleEvery(updateRate.getOrElse(webSocketConfig.throttleDuration))(
        TopsRequestMessages.foreach(kafkaPollerHub.topsPoller ! _)
      )
    ...
    case StartTotals(updateRate) =>
      ...
      val s = context.system.scheduleEvery(updateRate.getOrElse(webSocketConfig.throttleDuration))(
        TotalsRequestMessages.foreach(kafkaPollerHub.totalsPoller ! _)
      )
  ...
  private def boxedAirports(airports: AirportList, box: CoordinatesBox): AirportList = {
